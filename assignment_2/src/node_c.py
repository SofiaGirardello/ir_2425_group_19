...
class PickPlaceServer:
    ...
    def pick_and_place(self, id, object_pose, place_pose):
        ...
        above_object_pose.position.z = object_pose.pose.position.z + 0.3
        ...
        rospy.loginfo(f"Picking above object at: {above_object_pose}")

        success = self.move_arm_to_pose(above_object_pose)
        rospy.loginfo("Moved to object above pose")
        ...
        success = self.move_arm_to_pose(object_pose)
        ...
        self.navigate_to_pickup_table(8.7, -2.3, 3.14)

        try:
            self.tflistener.waitForTransform("base_link", "map", rospy.Time(0), rospy.Duration(1.0))
        except tf.Exception as e:
            rospy.logerr(f"Transform unavailable: {e}")
            return
        try:
    
            place_pose = self.tflistener.transformPose("base_link", place_pose)
            rospy.loginfo("Placement position transformed!")
        except tf.Exception as e:
            rospy.logerr(f"Failed to transform pose for tag ID {tag_id}: {e}")

        rospy.loginfo(f"Placement object at: {place_pose}")

        # 11. Place the object on the table
        place_pose.pose.position.z += 0.2
        # place_pose.pose.position.z = 1.1
        place_pose.pose.orientation = above_object_pose.orientation

        rospy.loginfo(f"Placing object at: {place_pose}")

        self.move_arm_to_pose(place_pose)

        place_pose.pose.orientation = object_pose.pose.orientation
        self.move_arm_to_pose(place_pose)
        ...
        self.attach_srv(detach_req.model_name_1, detach_req.link_name_1, detach_req.model_name_2, detach_req.link_name_2)
